...
You might find the discussion of vectors, elements, and operations very abstract - I did when I was learning R. While the list analogy is helpful, it only works for so long before it becomes problematic - because there's another data structure called **`lists`**. That confused me.

But what helped me understand vectors was the realisation that a vector is simply a "line of data." Imagine that we are running a study and we collect data on participants' age. When we open the Excel file, there will be a column called "age" with all the ages of my participants. That column is a vector of data with the variable label "age."

Creating that vector is the equivalent of creating a column in Excel (i.e. creating a vertical line of data):
...
Similarly, rows are lines of data going horizontally. If I add data to each column of our excel dataset, I am creating a new row (line) of data. This is the equivalent of doing the following in R.
...
So whenever you think of a vector, just remember that it refers to a line of data that will either be a column or a row.

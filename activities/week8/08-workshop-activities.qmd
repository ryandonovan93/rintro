...
In this week's workshop, we will learn how to run a **one-way between-subjects ANOVA** and a **factorial ANOVA** in R.
...
***ResearchQuestion***: Is there an effect of psychoactive treatment on Neuroticism?
...
If you remember from week 4, we can adjust the `descriptives()` function to remove unnecessary details and add visualizations.
...
Now, let's apply the syntax to our actual dataset, `df_psycho`:
...
4.  **`adjust = "bonferroni"`** Applies the Bonferroni correction to adjust p-values for multiple comparisons.

**Why Use the Bonferroni Correction?**

When conducting multiple comparisons, the risk of making a Type I error (false positive) increases. The Bonferroni correction reduces this risk by adjusting p-values to be more conservative. There are other types of corrections we can apply, but for this class we will stick with Bonferroni.
...
AOV1_pairwise <-emmeans(anova_one, pairwise ~ Treatment, adjust = "bonferroni")
...
-   **DMT vs. Placebo:** *p = 1.000* (no significant difference)
...
The analysis revealed a significant effect of treatment on Neuroticism, F(2, 57) = 9.05, p \< .001, η²p = .241. This suggests that approximately 24.1% of the variance in Neuroticism scores can be attributed to differences between treatment groups.

Post-hoc pairwise comparisons, using the Bonferroni correction, revealed that:

The Psilocybin group had significantly higher Neuroticism scores than the DMT group (t = -3.97, p = .0006).

The Psilocybin group also had significantly higher Neuroticism scores than the Placebo group (t = -3.31, p = .0049).

No significant difference was found between the DMT and Placebo groups (t = -0.66, p = 1.000).

These results suggest that Psilocybin led to significantly higher Neuroticism scores compared to DMT and Placebo, while DMT and Placebo did not differ significantly.
...
1.  **A priori power analysis** – Performed *before* data collection to determine the required sample size.\
...
We can see that our power is 0.37 (37%), which suggests it is substantially underpowered. We need to be extra cautious when interpreting these results.

### Step 8: Create a Nice Visualsation (Violin Plot)
...
ggplot(df_psycho, aes(x = Treatment, y = Neuroticism, fill = Treatment))+
  geom_violin(trim = FALSE)+
  geom_boxplot(width = .2) + 
  theme_classic()
```

The main thing, is that when you are adaptive this code is to replace Treatment with your Independent Variable, and Neuroticism with your Dependent Variable. Similarly, this particular code will only work for a One-Way Between Subjects ANOVA!
...
H0: There is no effect of Caffeine on Mood?

H1: There is an effect of Caffeine on Mood.
...
2.  **Explore the Data**
    ...
    -   Calculate descriptive statistics for `mood`grouped by `group`.

    -   Create visualisations: histograms with density plots, and boxplots.
...
5.  **Conduct Post-Hoc Comparisons**
    ...
    -   Use `emmeans()` with Bonferroni correction to determine which specifc groups are significant different.
...
7.  **Conduct a Power Analysis**
    ...
    -   Use `pwr.anova.test()` to check statistical power (post-hoc).

# Factorial ANOVA

## Activity 6: Factorial ANOVA Example

One-Way ANOVAS are a good test when you have one independent variable with three or more levels. However, most of the time you will probably have more than one IV. In those situations, Factorial ANOVAS can be excellent statistical tests to use. Let's go through a Factorial ANOVA now.
...
**Method**: A total of 180 participants were randomly assigned to one of three therapy groups: CBT (n = 60), DBT (n = 60), or Mindfulness (n = 60). They were also randomly assigned to receive therapy for either 6 weeks or 12 weeks. After completing the assigned intervention, their anxiety levels were measured.
...
Now, let’s run some descriptive statistics using the `descriptives()` function. This will help us understand the distribution of **Anxiety** scores across the different treatment groups.
...
Below is the general syntax (you will need to replace certain placeholders with the appropriate column names from your dataset):
...
We can see that the main effect of Therapy type on Anxiety scores was statistically significant, F(2, 174) = 143.76, p \< 0.001. This indicates that different therapy types led to significantly different anxiety levels after the intervention. The effect size (generalized eta squared) is 0.623, suggesting that approximately 62.3% of the variance in anxiety scores can be attributed to differences in therapy type.
...
leveneTest(Anxiety ~ Therapy * Duration, data = df_anxiety)  # Levene’s test for equal variances
...
The estimated marginal means (emmeans) indicate the average post-intervention anxiety scores for each therapy type at both treatment durations. Lower anxiety scores indicate greater effectiveness in reducing anxiety.
...
### Step 7: Write up your results APA style
...
There was a significant main effect of therapy type, F(2, 174) = 143.76, p < .001, η²ₚ = 0.623, indicating that anxiety scores differed significantly across therapy conditions. Post-hoc comparisons with Bonferroni correction showed that CBT (M = 50.7, SE = 1.19) and DBT (M = 47.8, SE = 1.19) were significantly more effective than Mindfulness (M = 63.9, SE = 1.19) at reducing anxiety after 6 weeks (p < .0001). Similarly, after 12 weeks, CBT (M = 40.6, SE = 1.19) and DBT (M = 38.5, SE = 1.19) remained significantly more effective than Mindfulness (M = 59.7, SE = 1.19) (p < .0001). However, there was no significant difference between CBT and DBT at either time point (p = 0.671 for 12 weeks; p = 0.243 for 6 weeks).
...
The therapy type by treatment duration interaction was statistically significant, F(2, 174) = 3.50, p = .032, η²ₚ = 0.039, indicating that the effectiveness of therapy type depended on treatment duration.

Post-hoc comparisons showed that CBT and DBT showed a larger reduction in anxiety from 6 to 12 weeks compared to Mindfulness. Specifically, CBT improved by 10.1 points and DBT by 9.3 points, whereas Mindfulness showed only a small reduction of 4.2 points between the two time points. This suggests that longer treatment durations were particularly beneficial for CBT and DBT, whereas Mindfulness did not benefit as much from an extended duration.

## Activity 7: Factorial ANOVA Exercise

### **Research Introduction**

Obesity is a growing health concern, and many people try different exercise and diet strategies to lose weight. However, it is unclear how different types of exercise and diets affect weight loss. This study aims to investigate the impact of **exercise type** (*None, Cardio, Strength Training*) and **diet type** (*Standard, High-Protein*) on weight loss.

### **Research Question**

Is there an effect of exercise type and diet method on weight loss?

### **Hypotheses**

-   **Null Hypothesis (H₀):** There is no effect of exercise and diet method on weight loss.

-   **Alternative Hypothesis (H₁):** There is an effect of exercise and diet method on weight loss.

### **Method**

A total of **180 participants** were randomly assigned to one of **six groups** in a **3 × 2 design**:
...
Each group had **30 participants**. The study lasted **12 weeks**, with participants in the **exercise groups attending three supervised sessions per week**. Participants followed dietary guidelines based on their assigned diet type. **Weight loss (kg) was recorded at the end of the study.**

### **Our Task**

Conduct a **factorial ANOVA** to test whether exercise type and diet type affect weight loss and if there is an interaction between them.

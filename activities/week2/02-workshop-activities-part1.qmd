---
title: "Week 2 - Activities (Part 1)"
format: 
    html:
      toc: true
      toc-depth: 4
...
![](images/set_up_working_directory.gif){fig-align="center"}
...
![](images/03-activity1-old-code.gif){fig-align="center"}
...
::: {.callout-note collapse="true" title="Faster Way to Create Script"}
...
![](images/03-activity3-script-set-up-new.gif){fig-align="center"}
...
![](images/03-activity4-running-code.gif){fig-align="center"}
:::
...
![](images/03-activity4-variable-example.gif){fig-align="center"}

Variables are really useful when we want to combine information to perform some operations. For example, like calculating the total and mean of someones scores on Extraversion.
...
We can also use combine different variable types (character, integer, numeric, logical). For example, by using the `paste()` function
...
::: {.callout-tip collapse="true" title="Hold up. Why are we use `=` here and not `<-` to create our column vectors?"}
...
Indexing does **not** modify the original data frame. To save changes, assign the result to a new variable:
...
2.  **Extracting Data**
    ...
    -   Using the `student_data` data frame, complete the following tasks:
        ...
        -   Extract the names and grades of the first two students using `[]` notation.

...
Before installing some packages, Windows users need to install **RTools**, and Mac users need **XCode Command Line Tools**. These are necessary to install certain packages that require compiling code.

***Note: If you are using Posit Cloud, you do not need to complete this step. Feel free to move onto the next section/exercises***
...
::: {.callout-note collapse="true" title="Instructions for Mac Users"}
#### Mac Users - Install XCode Command Line Tools

**Xcode Command Line Tools** is a collection of software tools for **macOS** that allows R users to compile packages from source. Some R packages include C, C++, or Fortran code that must be compiled before they can be installed. Since Windows does not come with built-in tools for compiling code, **Xcode Command Line Tools** provides the necessary compilers and libraries.

**How to Install Xcode Command Line Tools**

1.  Click the Search bar in the top right hand corner of your screen.

2.  Search for Terminal.

3.  Open the Terminal app.

4.  The following window should appear. Type in the following command exactly as it appears, and press Enter.

5.  A pop-up window will appear. Click Install and wait for it to finish.
...
Once finished, you should now be able to install R packages. Now you can move on to the exercises.
:::
...
1.  Install the package `jmv` in the R console using the following command
...
### Let's import the `psycho.csv` datasets
...
Just a note, this is the same set of activities that were in last week's sheet. But since we did not have time in the last session to go through data frames, I have pasted the exercises here. If you completed last week's activity on data frames and feel like you understood the content, then feel free to skip this section.
...
If you are using posit cloud, you will first need to download the file onto your computer before you can open it. To do that, go to Files pane, make sure you have the week3 folder opened. There you should be a tick box right next to the file `2025_exam_results.csv`. Tick that box. Then above where it says "Cloud \> project \> week3", you should see a toolbar with the button "More", click that button, click Export, and then click Download.
...
1.  Create Simple Functions (Write this code for yourself)
...
3.  Write a function called `kg_to_lbs` that takes in a single kg measurement as input and returns its conversion into pounds (lbs). Note: 1kg is equal to 2.20 pounds.
...
5.  Let's try write a multi-argument (i.e multi-input function). Write a function called `average_score()` that takes in two test scores (``` score1``, ```score2`) as arguments and returns their average (`avg\`). Test your function.

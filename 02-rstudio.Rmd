...
In the next section, I am going to show you how to download R and RStudio onto your laptop. But before we do that, I want you to set up a free account on Posit Cloud (formerly known as RStudio Cloud).

Posit Cloud enables you to run R and RStudio online for free, with no need to install anything. We are going to be using Posit Cloud as a back-up option in case any technical issues pop up. During class, we might not be able to solve that issue quickly and efficiently (in a large classroom, one must always account for Murphy's Law). If that does occur, then you can sign into Posit Cloud and keep following alone with the session. 

You might be wondering, why not just use Posit Cloud? The reason is that there are significant restrictions to the free version of Posit Cloud, namely that you can only use 25 hours per month and you are limited in the amount of data we can use during that time. Imagine working on your assignment, with all your files and data, and then hitting your max limit! This is why I recommend using Posit Cloud as a back-up. 

Similarly, the computers in the lab can be frustrating to use, particularly when you will be working with the same folder/files week by week. If you are using the computers in the lab and are having issues, feel free to pop over to Posit Cloud. 
...
Don't worry about what anything on the screen means for now. We'll come back to that once we download RStudio on your computer. 
